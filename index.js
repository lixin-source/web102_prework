...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++) {
        ...
        //img, name, description, pledged, goal, backers
        const gameDisplay = `
        <div class = "games">
...
raisedCard.innerHTML = `<p>${totalRaised.toLocaleString('en-US', {style: 'currency', currency: 'USD',minimumFractionDigits: 0})}</p>`;

// grab number of games card and set its inner HTML
const gamesCard = document.getElementById("num-games");

const totalGames = GAMES_JSON.reduce((acc, GAMES_JSON)=> {
    const num = parseInt(1,GAMES_JSON.name);
    return acc + num;
},0);
...
function filterUnfundedOnly() {
    ...
    const filteredStuff = GAMES_JSON.filter(game => game.pledged < game.goal);

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(filteredStuff);
}
...
first.innerHTML = one.name;
firstGameContainer.append(first);

// do the same for the runner up item
const second = document.createElement("p");
second.innerHTML = two.name;
